defmodule GameHubWeb.CoCaNgua.GameLive do
    ...
    case Board.roll(socket.assigns.board) do
      {:ok, board, result} ->
        {:noreply,
         socket
         |> push_history()
         |> assign(board: board, message: roll_message(result), roll_seq: socket.assigns.roll_seq + 1)
         |> refresh_legal_moves()}
  ...
  def render(assigns) do
    ...
    >
      ...
      <div class="relative flex w-32 shrink-0 flex-col gap-2 py-2 sm:w-40 sm:gap-3 lg:w-48">
        <div class="flex items-center justify-between">
          <button type="button" phx-click="toggle_settings" class="toy-icon-btn !h-10 !w-10 sm:!h-12 sm:!w-12" aria-label="Cài đặt">
        ...
        <div
          :if={@show_settings}
          class="absolute left-0 top-14 z-40 w-56 rounded-2xl border-2 p-2 shadow-xl sm:top-16"
          style="background: white; border-color: var(--cream-deep);"
        >
          <.link
            navigate={~p"/games/co-ca-ngua"}
            class="block rounded-xl px-3 py-2.5 text-sm font-bold hover:bg-[--cream]"
            style="color: var(--toy-ink);"
          >
            🔄 Chơi lại ván mới
          </.link>
          <.link
      ...
      <div class="flex min-w-0 flex-1 items-center justify-center">
        <div class="relative aspect-square h-full max-h-full max-w-full shrink-0">
          ...
          >
            <div
              style="display: grid; grid-template-columns: repeat(15, minmax(0, 1fr)); grid-template-rows: repeat(15, minmax(0, 1fr)); width: 100%; height: 100%;"
            >
              <div
              ...
              <button
                ...
                class={[
                  ...
                  selectable?(@legal_moves, @pending_move, p.color, p.idx) && "z-30 cursor-pointer animate-movable",
                  not selectable?(@legal_moves, @pending_move, p.color, p.idx) && "cursor-default"
                ]}
              >
                <img src={horse_src(p.color)} alt={Board.color_name(p.color)} class="h-full w-full object-contain drop-shadow" />
              </button>
              ...
              >
                <span
                  class="flex h-3/5 w-3/5 animate-piece-pop items-center justify-center rounded-full text-[10px] font-bold text-white shadow-lg ring-2 ring-white sm:text-xs"
                  style="background: var(--toy-blue-dark);"
                >
                  {step}
                </span>
              </div>
          ...
          >
            <div :for={c <- confetti_particles()} class="confetti-piece" style={confetti_style(c)}></div>
            <img src={trophy_src()} alt="" class="h-24 w-24 animate-trophy-pop object-contain drop-shadow-xl sm:h-32 sm:w-32" />
            <p class="text-2xl font-bold sm:text-3xl" style={"color: #{yard_ink(@board.winner)};"}>
      ...
      <div class="flex w-32 shrink-0 flex-col gap-2 py-2 sm:w-40 sm:gap-3 lg:w-48">
        <div class="flex items-center justify-end">
          <.link navigate={~p"/games/co-ca-ngua"} class="toy-icon-btn !h-10 !w-10 sm:!h-12 sm:!w-12" aria-label="Thoát">
        ...
        <div class="flex flex-col items-stretch gap-1.5">
          <div
            :if={@pending_move}
            class="rounded-2xl border-2 p-2 text-center"
            style="background: white; border-color: var(--toy-blue);"
          >
            <p class="text-xs font-bold" style="color: var(--toy-ink);">
              Di chuyển {move_description(@pending_move)}?
            </p>
            <p class="mt-0.5 text-[10px]" style="color: var(--toy-ink-soft);">
              Đi {length(@pending_move.path)} bước.
            </p>
            <div class="mt-1.5 flex flex-col gap-1.5">
              <button type="button" phx-click="confirm_move" class="toy-btn toy-btn--blue w-full !min-h-0 !py-1.5 !text-sm">
                Xác nhận
              </button>
              <button type="button" phx-click="cancel_move" class="toy-btn toy-btn--neutral w-full !min-h-0 !py-1.5 !text-sm">
                Hủy
              </button>
            </div>
          </div>
          ...
          <div :if={!@pending_move} class="flex flex-col items-stretch gap-1.5">
            <button
              ...
              type="button"
              phx-click={JS.push("roll") |> JS.transition({"ease-out duration-150", "scale-100", "scale-90"}, time: 150)}
              class="toy-btn toy-btn--red w-full !min-h-[52px] !text-sm sm:!min-h-[60px] sm:!text-lg"
            >
              🎲 Đổ xúc xắc
            </button>

            <div :if={@board.status == :moving} id={"die-#{@roll_seq}"} class="mx-auto flex h-11 w-11 items-center justify-center rounded-2xl border-2 p-1 animate-dice-pop sm:h-14 sm:w-14 sm:p-1.5" style="border-color: var(--toy-blue); background: white;">
              <img src={die_src(@board.die)} alt={"Xúc xắc #{@board.die}"} class="h-full w-full object-contain" />
            </div>

            <button type="button" phx-click="undo" disabled={@history == []} class="toy-btn toy-btn--neutral w-full !min-h-[40px] !text-xs sm:!min-h-[44px] sm:!text-sm">
              ↩️ Hoàn tác
            </button>
          </div>
        </div>
    ...
    <div class="rotate-prompt fixed inset-0 z-[100] flex-col items-center justify-center gap-4 text-center" style="background: var(--cream);">
        <span class="text-7xl animate-phone-rotate">📱</span>
        <p class="text-xl font-bold" style="color: var(--toy-ink);">Xoay ngang thiết bị để chơi nhé!</p>
        <p class="text-sm" style="color: var(--toy-ink-soft);">Cờ Cá Ngựa chơi đẹp nhất ở chế độ ngang 🐴</p>
      </div>
    """
  end
  ...
  defp player_panel(assigns) do
    ...
    <div class={[
      ...
      <div class={["relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full sm:h-14 sm:w-14", piece_backing(@player.color)]}>
        <img src={horse_src(@player.color)} alt="" class={["h-8 w-8 object-contain sm:h-11 sm:w-11", @active && "animate-avatar-bounce"]} />
      </div>
      <div class="min-w-0 flex-1">
        <p class="break-words text-xs font-bold leading-tight sm:text-sm" style="color: var(--toy-ink);">{@player.name}</p>
        <p class="text-[10px] sm:text-xs" style="color: var(--toy-ink-soft);">{@finished}/4 🏁</p>
      </div>
    </div>
    """
  end
  ...
  defp yard_style(color, current_color) do
    ...
    if color == current_color, do: base <> " --yard-glow-color: #{yard_glow_color(color)};", else: base
  end
  ...
  # Each 13-cell arm of the loop is tinted with the color whose start it begins at.
  defp arm_color(idx), do: Enum.at(Layout.colors(), div(idx, 13))
